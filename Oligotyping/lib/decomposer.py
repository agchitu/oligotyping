...
class Decomposer:
    def __init__(self, args=None):
        ...
        #  set number of processes and threads to be used
    ...
    def check_dirs(self):
        ...
        if not self.output_directory:
            self.output_directory = os.path.join(os.getcwd(),
                                                 '-'.join([self.project.replace(' ', '_'), self.get_prefix()]))
        ...
        if not os.access(self.output_directory, os.W_OK):
            raise utils.ConfigError(
                "You do not have write permission for the output directory: '%s'" % self.output_directory)
    ...
    def _init_topology(self):
        ...
        if self.root.size < self.min_actual_abundance:
            raise utils.ConfigError(
                "The number of reads in alignment file (%d) is smaller than --min-actual-abundance (%d)" % \
    ...
    def decompose(self):
        ...
        self.run.info('multi_processes', not self.no_multi_processes)
            ...
            for node_id in self.node_ids_to_analyze:
                ...
                if node.reads[0].frequency < self.min_substantive_abundance:
                    if node.node_id == 'root':
                        self.progress.end()
                        raise utils.ConfigError(
                            "Number of unique reads in the root node (%d) is less than the declared minimum (%d)." \
                ...
                # node.do_entropy()   #  agc
                if ((node_id is 'root') and node.dirty) or (node_id is not 'root'):
                    node.do_entropy_agc()  # agc
                ...
                while 1:
                    ...
                    oligo = ''.join([chr(read.seq[d]) for d in node.discriminants])
        ...
        if self.no_multi_processes:
            for node in dirty_nodes:
                self.progress.update(
                    'Synchronizing dirty nodes (%d of %d)' % (dirty_nodes.index(node) + 1, len(dirty_nodes)))
                node.refresh()
        else:
            def worker_refresh_topology(data_chunk, shared_counter, results_array):
                for node in data_chunk:
                    node.refresh()
                    results_array.append(node)
                    shared_counter.set(shared_counter.value + 1)

            mp = utils.Multiprocessing(worker_refresh_topology, self.number_of_processes)
            ...
            while 1:
                ...
                if not num_processes:
                    # all processes are done. replace nodes with resulting nodes.
                    for node in results_array:
                        self.topology.nodes[node.node_id] = node
                    break

                self.progress.update('Processing in %d processes: %d of %d' % (num_processes,
    ...
    def _relocate_outliers(self, reason, refresh_final_nodes=True):
        ...
        for _id in similarity_dict:
            counter += 1
            self.progress.update('Relocating outliers: %d of %d' % (counter, num_outlier_objects))
    ...
    def _refresh_final_nodes(self):
        ...
        for i in range(0, len(self.topology.final_nodes)):
            ...
            self.progress.update('Processing %d of %d (current size: %d)' %
                                 (i + 1, len(self.topology.final_nodes), node.size))
            node.refresh()
    ...
    def _store_read_distribution_table(self):
        ...
        def get_dict_entry_tmpl():
            ...
            for _reason in self.topology.outlier_reasons:
                d[_reason] = 0
            return d
    ...
    def _store_final_nodes(self):
        ...
        if self.no_multi_processes:
            ...
            def worker_store_final_nodes(node_id):
                _node = self.topology.get_node(node_id)
                _node.store()

            mp = utils.Multiprocessing(worker_store_final_nodes, self.number_of_processes)
    ...
    def _get_unit_counts_and_percents(self):
        ...
        self.unit_counts, self.unit_percents = utils.get_unit_counts_and_percents(self.topology.final_nodes,
                                                                                  self.samples_dict)
    ...
    def _store_topology(self):
        ...
        for node_id in self.topology.alive_nodes:
            ...
            nodes_dict[node_id] = {'size': node.size,
                                   ...
                                   'num_comps_larger_than_m': len(
                                       [True for tpl in node.entropy_tpls if tpl[1] > self.min_entropy]),
    ...
    def _store_node_representatives(self):
        ...
        with open(node_representatives_file_path, 'w') as f:
            self.progress.update('Being generated')
            for node_id in self.topology.final_nodes:
                node = self.topology.get_node(node_id)
    ...
    def _generate_exclusive_figures(self):
        ...
        pickle.dump(exclusive_figures_dict, open(exclusive_figures_dict_file_path, 'wb'))
