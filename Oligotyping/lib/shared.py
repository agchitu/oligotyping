...
def generate_default_figures(_object):
    ...
    for (analysis, script, output_dir) in [('Stackbar', 'o-stackbar.R', 'stackbar')]:
        ...
        figures_dict['basic_reports'][output_dir] = dict()
...
def generate_exclusive_figures(_object):
    ...
    for category in sample_mapping_dict:
        ...
        # double filter: first makes sure the sample was not removed from the analysis due to losing all its reads 
        # during the refinement, second makes sure that sample was actually mapped to something 
        # in the sample mapping file.
        ...
        if samples == _object.samples:
            ...
            matrix_percent_path = get_temporary_file_name('%s-' % category, '-matrix-percent.txt',
        ...
        for (analysis, script, output_dir) in [
            ('NMDS Analysis', 'o-metaMDS-analysis-with-metadata.R', 'nmds_analysis')]:
