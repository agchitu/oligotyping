...
def decomposer():
    ...
    parser.add_argument('-m', '--min-entropy', type=float, default=0.0965, metavar="FLOAT",
                        help='Minimum entropy for a component to have in order to be picked as a\
                                discriminant. Default: %(default)f')
    ...
    parser.add_argument('-d', '--number-of-discriminants', type=int, default=4, metavar="INTEGER",
                        help='Number of discriminant locations to be used for entropy decomposition\
                                discriminant. Default: %(default)d')
    parser.add_argument('-A', '--min-actual-abundance', type=int, default=0, metavar="INTEGER",
                        help='Minimum number of reads in a node for decomposition to continue. Decomposition\
                                ...
                                chosen carefully depending on the size of the sample. Although this parameter\
    ...
    parser.add_argument('-t', '--sample-name-separator', type=str, default='_', metavar="CHAR",
                        help='Character that separates sample name from unique info in the def line. For instance\
                                if the def line says >sample-1_GD7BRW402IVMZE, the separator should be set to "_"\
    ...
    parser.add_argument('-P', '--number-of-processes', type=int, default=0, metavar="INTEGER",
                        help='Number of processes to use when possible to parallelize. '
                             'It is a good idea to keep this number smaller than the number '
                             'of CPU cores available. If set to 0, this number will be set to 90%% of available cores,'
