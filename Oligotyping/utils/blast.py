...
class LocalBLAST:
    ...
    def get_results_dict(self, mismatches=None, gaps=None, min_identity=None, max_identity=None,
                         penalty_for_terminal_gaps=True):
        results_dict = {}
        ...
        while b6.next():
            ...
            if penalty_for_terminal_gaps:
                ...
                if b6.entry.q_start != 1 or b6.entry.s_start != 1:
                    additional_gaps += (b6.entry.q_start - 1) if b6.entry.q_start > b6.entry.s_start else (
                                b6.entry.s_start - 1)
                if additional_gaps != b6.entry.q_len or b6.entry.s_end != b6.entry.s_len:
                    additional_gaps += (b6.entry.q_len - b6.entry.q_end) if (b6.entry.q_len - b6.entry.q_end) > (
                                b6.entry.s_len - b6.entry.s_end) else (b6.entry.s_len - b6.entry.s_end)
    ...
    def get_fancy_results_dict(self, max_per_query=10, defline_white_space_mask=None):
        ...
        while next(b6):
            ...
            hsp_match = ''.join(
                ['|' if query_aligned[i] == target_aligned[i] else ' ' for i in range(0, len(query_aligned))])
