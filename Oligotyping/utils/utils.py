...
def homopolymer_indel_exists(seq1, seq2):
    ...
    isHPindel = lambda s_e: seq1[s_e[0]:s_e[1]] == seq2[s_e[0]:s_e[1]] and isHP(seq1[s_e[0]:s_e[1]]) == 1 and seq2[
        gap_index] == seq2[s_e[0]]
...
def generate_gexf_network_file(units, samples_dict, unit_percents, output_file, sample_mapping_dict=None,
    ...
    sample_mapping_categories = sorted(
        [k for k in list(sample_mapping_dict.keys()) if k != 'colors']) if sample_mapping_dict else None
    unit_mapping_categories = sorted(
        [k for k in list(unit_mapping_dict.keys()) if k not in ['colors', 'labels']]) if unit_mapping_dict else None
...
def generate_gexf_network_file_for_nodes_topology(nodes_dict, output_file, attribute_types_dict={}, project=None,
                                                  skip_node_labels=False):
    output = open(output_file, 'w')

    nodes = sorted(nodes_dict.keys())
    nodes_mapping_categories = sorted(
        [k for k in list(nodes_dict[nodes[0]].keys()) if k not in ['children', 'parent']]) if nodes_dict else None
...
def process_command_line_args_for_quality_files(args, _return='qual_stats_dict', verbose=True):
    ...
    if args.qual_scores_file:
        ...
        qual_stats_dict = get_qual_stats_dict(quals_dict, \
                                              output_file_path=args.qual_scores_dict.split('.cPickle')[
                                                                   0] + '.STATS.cPickle')
...
def get_read_objects_from_file(input_file_path, progress_obj=None):
    input_fasta = u.SequenceSource(input_file_path, unique=True, progress_obj=progress_obj)
    read_objects = []
    ...
    del input_fasta
    progress_obj.update('Cleanup memory %d' % gc.collect())
...
class UniqueFASTAEntry:  # agc
    def __init__(self, seq, ids, md5id=None):
        self.seq = seq.encode('utf-8') if not isinstance(seq, bytes) else seq
        self.ids = ids
